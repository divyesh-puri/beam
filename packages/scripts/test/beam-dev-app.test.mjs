...
const launcherPath = join(repoRoot, "scripts", "beam-dev-app");
...
describe("beam-dev-app", () => {
  it("keeps the caller's Node when another Node 22 version is installed", () => {
    const tempRoot = mkdtempSync(join(tmpdir(), "beam-dev-node-"));
    try {
      ...
      expect(result.stdout).toContain(
        `Data dir: ${join(tempRoot, ".beam-dev")}/`,
      );
    } finally {
