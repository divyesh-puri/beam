...
function reserveProductionPorts(port: number): number {
  ...
  if (port === 48_886) return 59_002;
  if (port === 48_887) return 59_003;
  return port;
}
...
export function expectedDevPorts(repoRoot: string): ExpectedDevPortSet {
  ...
  return {
    ...
    cloudPort: reserveProductionPorts(35_000 + offset),
    cloudWorkerPort: reserveProductionPorts(43_000 + offset),
...
export function expectedDevDataDir(args: ExpectedDevInstanceArgs): string {
  return join(args.homeDir, ".beam-dev", expectedDevInstanceId(args));
}
