...
} from "../../../scripts/start-beam.mjs";

const testDir = dirname(fileURLToPath(import.meta.url));
const repoRoot = resolve(testDir, "..", "..", "..");
const startBeamUrl = pathToFileURL(
  ...
  try {
    return await Promise.race([
      ...
      new Promise((_, rejectPromise) => {
        timeout = setTimeout(
          () => rejectPromise(new Error("start-beam fixture did not stop")),
          timeoutMs,
        );
      }),
    ]);
...
describe("start-beam", () => {
  it("keeps the worktree policy marker out of bb-app arguments", () => {
    ...
    expect(parseStartBeamArgs(["--server-port", "4000"])).toEqual({
      cliArgs: ["--server-port", "4000"],
  ...
  it("uses a fresh process after a native binary changes", async () => {
    const fixtureRoot = mkdtempSync(join(tmpdir(), "beam-native-preflight-"));
  ...
  posixIt(
    ...
    async () => {
      const fixtureSource = [
        `import { runBuildProcess } from ${JSON.stringify(startBeamUrl)};`,
  ...
  posixIt(
    ...
    async () => {
      const fixtureRoot = mkdtempSync(join(tmpdir(), "beam-native-signal-"));
      scratchDirs.push(fixtureRoot);
      ...
      const fixtureSource = [
        `import { runNativeModulePreflight } from ${JSON.stringify(startBeamUrl)};`,
        "try {",
