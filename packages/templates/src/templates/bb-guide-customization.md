---
kind: instruction
title: Beam Guide — Customization
summary: Command reference for customizing the Beam app color palette and keyboard shortcuts.
intent: Explain the CLI theme surface and server-backed app customization.
editingNotes: Keep flags accurate against the CLI implementation. Theme details live in the beam-cli skill's references/theming.md.
---
Customization commands

Theming — the app-wide color palette

`beam theme` controls a set of CSS-variable overrides, persisted server-side and
...
disk, one folder per theme, at <beam-data-dir>/theme/<name>/theme.css (the packaged
app uses ~/.beam/theme/…). The folder name is the theme id.

  beam theme list                  Built-in and custom themes; shows the active one
  beam theme dir                   Print the custom-theme directory (where to author)
  beam theme set <id> [--favicon-color <color>]
                                 Activate a theme, preserving the favicon color
                                 unless the flag supplies the complete selection
  beam theme show [--css]          Print the active palette; --css dumps the CSS
  beam theme reset                 Back to the default theme; preserve favicon color
  beam theme favicon set <color>   Set favicon color; preserve the active theme
  beam theme favicon reset         Reset favicon color; preserve the active theme

To author a custom theme, run `beam theme dir`, write <that-dir>/<name>/theme.css,
then `beam theme set <name>`. Optional `pierre-dark.json` / `pierre-light.json`
...
the beam-cli skill (references/theming.md).
...
`beam config` and `beam env` reload runtime settings in a running server,
...
`beam config`, not `beam env`, to change `BB_APP_URL`, `BB_INFERENCE`,
`BB_INFERENCE_FALLBACK`, or `BB_TRANSCRIPTION` live. After a startup-only
change, run `beam stop && beam start` or restart the desktop app. Until
...
`codex/gpt-5.4-mini`, respectively.
  ...
  beam config set BB_INFERENCE <provider/model>
  beam config set BB_INFERENCE_FALLBACK <provider/model>
...
`beam keep-awake enable` or `beam keep-awake disable`. Choose every host with `beam
keep-awake hosts all`, or name individual host ids after `beam keep-awake hosts`.
On macOS it prevents system idle sleep while Beam is running; closing the lid or
...
false in packaged builds. Turn it on to show raw provider events Beam does not yet
...
it on to hide every `customModels` entry from `~/.beam/config.json` in all model
lists (pickers, `beam provider models`, and the SDK) during a screen share. The
entries stay in the config file.

  beam settings show
  beam settings ai-services
  beam settings general <key> <value>
  beam settings experiment <key> <value>
  beam settings usage [--machine <id-or-name>]
  beam settings version [--force]
  beam settings reload

`beam settings ai-services` shows the helper-inference and voice-transcription
settings (`BB_INFERENCE`, `BB_INFERENCE_FALLBACK`, `BB_TRANSCRIPTION`, set with
`beam config`) and the plugin-registered AI services they may name as
`<service>/<model>`.

`beam settings general` accepts any key from `generalSettings` in
`beam settings show`. Boolean preferences take `true`, `false`, `on`, or `off`,
...
every restorable provider. Beam releases those sessions after 30 idle minutes.
The daemon applies a changed value within five minutes. Active turns, commands,
agents, workflows, and monitors keep their sessions loaded. Beam releases idle
...
`beam settings experiment timelineWindowing true`.
...
Reset removes an override and returns to Beam's current default; Clear explicitly
...
same resolved bindings. The complete default table is in docs/configuration.md.
  ...
  beam settings keyboard list
  beam settings keyboard hints <true|false>
  beam settings keyboard set <command> <shortcut|disabled>
  beam settings keyboard reset [command]

Host files and voice transcription

  beam file read|write|list|paths|mkdir|move|remove ...
  beam voice transcribe <audio-file> [--prompt <context>]

Voice transcription uses the `BB_TRANSCRIPTION` model, which defaults to
`codex/gpt-transcribe`. Override it with
`beam config set BB_TRANSCRIPTION <provider/model>`.

`beam file` supports `--host` for remote machines and `--root` on mutating
...
localStorage as `bb.voiceInput.audioInputDeviceId`; it does not have a `beam`
