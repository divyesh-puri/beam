...
Providers are agent backends (e.g., codex, claude-code). Each supports different models.
  ...
  beam provider list [--machine <id-or-name> | --environment <id>]
                                          List available providers
  beam provider models [providerId] [--machine <id-or-name> | --environment <id>]
...
When provider and model are omitted from beam thread spawn, the project's
remembered defaults apply. If the project has no remembered choice, Beam uses
...
an active turn. These settings are separate from Beam's optional Memory plugin,
an official plugin bundled with the app.

Provider-native subagents can also be disabled on those provider pages. For
Codex, Beam turns off the native multi-agent feature and caps V2 sessions at the
root thread so remote session policy cannot start a child. For Claude Code, Beam
...
settings, or run:
  ...
  beam provider-retry status [thread-id] [--json]    Inspect in-memory waits
  beam provider-retry cancel <thread-id> [--json]    Cancel an automatic retry
  beam provider-retry retry <thread-id> [--json]     Request a manual retry

Timed waits exist only while the current Beam server/plugin process remains
running. Disabling/reloading the plugin or restarting the server clears them;
the original failed thread remains available for `beam provider-retry retry`.
...
Beam indexes the native user and project skill roots for Codex, Claude Code, Pi,
...
command menu. `beam skill list` shows native skills for Claude Code, Codex, and
Cursor.
...
appears automatically. Discover and select one with:
  ...
  beam provider models acp-opencode --environment "$BB_ENVIRONMENT_ID"
  beam thread spawn --provider acp-opencode --model <provider/model>

Beam applies the selected model to the ACP session before the first prompt.
...
session mode, not a model. Beam does not select OpenCode agents; configure the
...
provider id), a model id, and an optional displayName. Beam skips an invalid
entry with a warning. The entry then appears in Beam provider models output and
...
model to the OpenCode config instead. Edit the JSON and run beam config
...
JSON array. Set it with beam plugin config provider-acp set customAgents '[...]'.
Each entry needs id (lowercase letters, digits and dashes), displayName, and
command. Beam derives provider id acp-<id> from the slug id. The id is permanent.
The id cursor is reserved because Beam always lists that agent. The ids opencode,
...
the vendor side channels Beam reads. Use nativeSkillRoots to add native skills to
the composer.
...
Beam hides the /compact command for agents that do not declare it. The plugin
re-registers its providers as soon as the setting changes, so no restart or
config refresh is needed.

The old customAcpAgents array in the app data-dir config.json is deprecated. Beam
...
setting does not accept: Beam drops that field when it reads the old array, and a
...
user and project paths use the same relative-path rules. Beam indexes these roots
as read-only sources. It then injects the selected skills into all providers.
The Beam user and project roots keep higher precedence than matching shared roots.
