...
editingNotes: Keep scope selectors and ID-only commands aligned with Beam terminal --help.
---
Terminal commands
...
real persistent PTY and appears in the Beam UI.

List and create require exactly one explicit scope:

  beam terminal list --thread <thread-id>
  beam terminal list --environment <environment-id>
  beam terminal list --machine <id-or-name> [--cwd <path>]

  beam terminal create --thread <thread-id> --command "pnpm dev"
  beam terminal create --environment <environment-id>
  beam terminal create --machine <id-or-name> [--cwd <path>]
...
All other operations need only the terminal ID:
  ...
  beam terminal show <terminal-id>
  beam terminal attach <terminal-id>        Ctrl-B d detaches
  beam terminal send <terminal-id> --text <text> [--enter]
    --stdin                               Read bytes from stdin instead of --text
  beam terminal resize <terminal-id> --cols <n> --rows <n>
  beam terminal rename <terminal-id> <title>
  beam terminal restart <terminal-id>       Atomically replaces it with a shell; does not replay the original command
  beam terminal close <terminal-id> [--if-clean]

  beam terminal output <terminal-id>
  ...
  beam terminal wait <terminal-id>
...
For a dev server, prefer:
  ...
  beam terminal wait <terminal-id> --contains "Local:" --timeout 120
