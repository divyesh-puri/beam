...
/**
 * `@get-bb/plugin-sdk/app` — typed facade over the Beam app's plugin runtime.
 *
 * This module's runtime is never bundled into plugins: `beam plugin build`
 * swaps the specifier for a shim reading
 * `globalThis.__bbPluginRuntime.pluginSdkApp` (which the Beam app fills with
 ...
 * Beam registry (`npx shadcn add @bb/<name>`); `toast` comes from
