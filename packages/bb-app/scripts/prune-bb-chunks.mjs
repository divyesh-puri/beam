...
/**
 * Delete the Beam CLI chunks that host-daemon/dist/bb does not reach.
 *
...
for (const [label, pathToCheck] of [
  ["bundled Beam CLI", entry],
  ["bundled Beam CLI chunks", chunkDir],
]) {
  try {
...
if (removed.length > 0) {
  ...
  process.stderr.write(
    `bb-app: pruned ${removed.length} stale Beam CLI chunk file(s)\n`,
  );
}
