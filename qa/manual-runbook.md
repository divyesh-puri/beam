...
This runbook covers the thorough non-app end-to-end validation pass for Beam's
...
- Thread recovery is validated with the existing lifecycle commands:
  `beam thread stop`, `beam thread tell`, `beam thread spawn`, archive/unarchive, and
  the recovery checks below. When the provider-retry plugin is enabled,
  `beam provider-retry retry <thread-id>` is the manual path for a failed,
  accepted provider rate-limit turn; inspect the thread before using it. For
  other failed or interrupted threads, send a fresh turn with `beam thread tell`,
  or create a replacement with `beam thread spawn` when a new thread is the right
  recovery path.
...
beam() { env -u BB_CLI node apps/cli/dist/index.js "$@"; }
```
...
PI_MODELS_JSON=$(beam provider models pi --json)
...
```bash
SMOKE_THREAD_ID=$(beam thread spawn \
...
beam thread wait "$SMOKE_THREAD_ID" --status idle --timeout 120
beam thread show "$SMOKE_THREAD_ID"
beam thread output "$SMOKE_THREAD_ID"
beam thread log "$SMOKE_THREAD_ID" --format json | jq '.[-10:]'
```

Send a follow-up after idle:

```bash
beam thread tell "$SMOKE_THREAD_ID" "Now say goodbye from the smoke pass"
beam thread wait "$SMOKE_THREAD_ID" --status idle --timeout 120
beam thread output "$SMOKE_THREAD_ID"
```
...
PROTOCOL_PARENT_ID=$(beam thread spawn \
  --project "$BB_PROJECT_ID" \
  --provider codex \
  --model "$CODEX_MODEL" \
  --reasoning-level low \
...
beam thread wait "$PROTOCOL_PARENT_ID" --status idle --timeout 240

PROTOCOL_CHILD_ID=$(beam thread spawn \
  --project "$BB_PROJECT_ID" \
...
beam thread wait "$PROTOCOL_CHILD_ID" --status idle --timeout 240
beam thread show "$PROTOCOL_PARENT_ID" --json | jq '.thread | {id, parentThreadId, providerId, status}'
beam thread show "$PROTOCOL_CHILD_ID" --json | jq '.thread | {id, parentThreadId, providerId, status}'
beam thread output "$PROTOCOL_CHILD_ID"
if beam manager list; then
  echo "expected beam manager list to fail"
  exit 1
fi
beam manager list 2>&1 | rg "Managers were replaced by parent threads|beam thread"
...
- `beam manager list` exits non-zero with a parent-thread replacement message
...
```bash
WORKTREE_THREAD_ID=$(beam thread spawn \
  --project "$BB_PROJECT_ID" \
  --provider codex \
  --model "$CODEX_MODEL" \
  --reasoning-level low \
...
beam thread wait "$WORKTREE_THREAD_ID" --status idle --timeout 120
WORKTREE_ENV_ID=$(curl -fsS "$BB_SERVER_URL/api/v1/threads/$WORKTREE_THREAD_ID" | jq -r '.environmentId')

beam thread show "$WORKTREE_THREAD_ID"
beam thread output "$WORKTREE_THREAD_ID"
beam thread show "$WORKTREE_THREAD_ID" --work-status
beam thread show "$WORKTREE_THREAD_ID" --git-diff --diff-target uncommitted
beam thread show "$WORKTREE_THREAD_ID" --git-diff --diff-target branch_committed
beam thread show "$WORKTREE_THREAD_ID" --git-diff --diff-target all
...
beam environment commit "$WORKTREE_ENV_ID" --json | jq -e '.action == "commit" and (.commitSha | type == "string")'
```
...
beam environment update "$WORKTREE_ENV_ID" --merge-base-branch "$MERGE_BASE_BRANCH"
beam environment show "$WORKTREE_ENV_ID" --json | jq -e --arg branch "$MERGE_BASE_BRANCH" '.mergeBaseBranch == $branch'
beam thread show "$WORKTREE_THREAD_ID" --work-status --git-diff --diff-target all

beam environment update "$WORKTREE_ENV_ID" --clear-merge-base-branch
beam environment show "$WORKTREE_ENV_ID" --json | jq -e '.mergeBaseBranch == null'
```

Archive and unarchive the smoke thread:

```bash
beam thread archive "$SMOKE_THREAD_ID"
curl -fsS "$BB_SERVER_URL/api/v1/threads/$SMOKE_THREAD_ID" | jq

if beam thread tell "$SMOKE_THREAD_ID" "This should fail while archived"; then
...
beam thread unarchive "$SMOKE_THREAD_ID"
beam thread tell "$SMOKE_THREAD_ID" "Say something after unarchive"
beam thread wait "$SMOKE_THREAD_ID" --status idle --timeout 120
beam thread output "$SMOKE_THREAD_ID"
```
...
```bash
DIRTY_ARCHIVE_THREAD_ID=$(beam thread spawn \
  --project "$BB_PROJECT_ID" \
  --provider codex \
  --model "$CODEX_MODEL" \
  --reasoning-level low \
...
beam thread wait "$DIRTY_ARCHIVE_THREAD_ID" --status idle --timeout 120
...
beam thread show "$DIRTY_ARCHIVE_THREAD_ID" --work-status

beam thread archive "$DIRTY_ARCHIVE_THREAD_ID"
...
beam thread delete "$DIRTY_ARCHIVE_THREAD_ID" --yes
...
- `beam environment commit` succeeds with helper-generated commit text without requiring `OPENAI_API_KEY`.
- Environment merge-base metadata can be set, reflected by `beam environment show`, used by thread status/diff output, and cleared.
- Archiving blocks `beam thread tell`; unarchiving restores normal operation.
...
beam thread wait "$THREAD_B_ID" --status idle --timeout 120
...
beam thread output "$THREAD_B_ID"
```

Alternate follow-ups across the two sibling threads:

```bash
beam thread tell "$THREAD_A_ID" "Say exactly: FOLLOW UP A"
beam thread wait "$THREAD_A_ID" --status idle --timeout 120

beam thread tell "$THREAD_B_ID" "Say exactly: FOLLOW UP B"
beam thread wait "$THREAD_B_ID" --status idle --timeout 120

beam thread output "$THREAD_A_ID"
beam thread output "$THREAD_B_ID"
beam thread log "$THREAD_A_ID" --format json | jq '.[-8:]'
beam thread log "$THREAD_B_ID" --format json | jq '.[-8:]'
```

Archive thread A and verify thread B still works:

```bash
beam thread archive "$THREAD_A_ID"
beam thread tell "$THREAD_B_ID" "Say exactly: STILL WORKING"
beam thread wait "$THREAD_B_ID" --status idle --timeout 120
beam thread output "$THREAD_B_ID"
beam thread unarchive "$THREAD_A_ID"
```

Run a mixed-provider pass in separate environments:

```bash
CLAUDE_THREAD_ID=$(beam thread spawn \
  --project "$BB_PROJECT_ID" \
...
PI_THREAD_ID=$(beam thread spawn \
  --project "$BB_PROJECT_ID" \
  --provider pi \
  --model "$PI_MODEL" \
  --reasoning-level low \
...
beam thread wait "$CLAUDE_THREAD_ID" --status idle --timeout 120
beam thread wait "$PI_THREAD_ID" --status idle --timeout 180
...
beam thread output "$CLAUDE_THREAD_ID"
beam thread output "$PI_THREAD_ID"
```
...
curl -fsS "$BB_SERVER_URL/api/v1/hosts" | jq
beam thread tell "$SMOKE_THREAD_ID" "Check recovery after daemon restart"
beam thread wait "$SMOKE_THREAD_ID" --status idle --timeout 120
beam thread output "$SMOKE_THREAD_ID"
```
...
PROVISION_RETRY_THREAD_ID=$(beam thread spawn \
  --project "$BB_PROJECT_ID" \
  --provider codex \
  --model "$CODEX_MODEL" \
  --reasoning-level low \
...
beam thread wait "$PROVISION_RETRY_THREAD_ID" --status error --timeout 120
...
beam thread log "$PROVISION_RETRY_THREAD_ID" --format json \
...
beam thread wait "$PROVISION_RETRY_THREAD_ID" --status idle --timeout 180
beam thread output "$PROVISION_RETRY_THREAD_ID"
...
if beam thread tell "$SMOKE_THREAD_ID" "This should fail while the host is offline"; then
...
eval "$RESTART_DAEMON_COMMAND"
DAEMON_PID=$(cat "$DAEMON_RESTART_PID_PATH")
beam thread tell "$SMOKE_THREAD_ID" "Say exactly: offline retry ok"
beam thread wait "$SMOKE_THREAD_ID" --status idle --timeout 120
beam thread output "$SMOKE_THREAD_ID"
```
...
```bash
HOT_REPLACE_THREAD_ID=$(beam thread spawn \
  --project "$BB_PROJECT_ID" \
  --provider codex \
  --model "$CODEX_MODEL" \
  --reasoning-level low \
...
beam thread wait "$HOT_REPLACE_THREAD_ID" --status active --timeout 30
...
curl -fsS "$BB_SERVER_URL/api/v1/hosts" | jq
beam thread show "$HOT_REPLACE_THREAD_ID"
beam thread log "$HOT_REPLACE_THREAD_ID" --format json | jq '.[-12:]'
```
...
```bash
beam thread tell "$SMOKE_THREAD_ID" "Write 80 detailed bullet points about the history of computing."
beam thread wait "$SMOKE_THREAD_ID" --status active --timeout 30
...
beam thread output "$SMOKE_THREAD_ID"
beam thread log "$SMOKE_THREAD_ID" --format json | jq '.[-12:]'
beam thread log "$SMOKE_THREAD_ID" --format json \
...
```bash
PROVIDER_THREAD_ID=$(beam thread spawn \
  --project "$BB_PROJECT_ID" \
...
beam thread wait "$PROVIDER_THREAD_ID" --status idle --timeout 120
beam thread output "$PROVIDER_THREAD_ID"

beam thread tell "$PROVIDER_THREAD_ID" "Repeat the previous answer in uppercase"
beam thread wait "$PROVIDER_THREAD_ID" --status idle --timeout 120
beam thread output "$PROVIDER_THREAD_ID"

beam thread tell "$PROVIDER_THREAD_ID" "Write a very long essay about computing history"
beam thread wait "$PROVIDER_THREAD_ID" --status active --timeout 30
beam thread stop "$PROVIDER_THREAD_ID"
beam thread wait "$PROVIDER_THREAD_ID" --status idle --timeout 120
beam thread show "$PROVIDER_THREAD_ID"
beam thread log "$PROVIDER_THREAD_ID" --format json | jq '.[-10:]'
```

For workspace interaction, repeat on a worktree thread:

```bash
PROVIDER_WORKTREE_THREAD_ID=$(beam thread spawn \
  --project "$BB_PROJECT_ID" \
  --provider <provider-id> \
  --model <provider-model> \
  --reasoning-level low \
...
beam thread wait "$PROVIDER_WORKTREE_THREAD_ID" --status idle --timeout 120
PROVIDER_WORKTREE_ENV_ID=$(curl -fsS "$BB_SERVER_URL/api/v1/threads/$PROVIDER_WORKTREE_THREAD_ID" | jq -r '.environmentId')

beam thread output "$PROVIDER_WORKTREE_THREAD_ID"
...
APPROVAL_THREAD_ID=$(beam thread spawn \
  --project "$BB_PROJECT_ID" \
  --provider codex \
  --model "$CODEX_MODEL" \
  --reasoning-level low \
...
for _ in {1..60}; do
  APPROVAL_INTERACTION_ID=$(beam thread interactions list "$APPROVAL_THREAD_ID" --json | jq -r '.[0].id // empty')
...
beam thread interactions show "$APPROVAL_INTERACTION_ID" "$APPROVAL_THREAD_ID"

if beam thread tell "$APPROVAL_THREAD_ID" "This should be blocked while an interaction is pending"; then
...
beam thread interactions approve "$APPROVAL_INTERACTION_ID" "$APPROVAL_THREAD_ID"
beam thread wait "$APPROVAL_THREAD_ID" --status idle --timeout 180
beam thread output "$APPROVAL_THREAD_ID"
beam thread interactions list "$APPROVAL_THREAD_ID" --json | jq
...
DENY_THREAD_ID=$(beam thread spawn \
  --project "$BB_PROJECT_ID" \
  --provider codex \
  --model "$CODEX_MODEL" \
  --reasoning-level low \
...
DENY_INTERACTION_ID=
for _ in {1..60}; do
  DENY_INTERACTION_ID=$(beam thread interactions list "$DENY_THREAD_ID" --json | jq -r '.[0].id // empty')
...
beam thread interactions show "$DENY_INTERACTION_ID" "$DENY_THREAD_ID"
beam thread interactions deny "$DENY_INTERACTION_ID" "$DENY_THREAD_ID"
if beam thread wait "$DENY_THREAD_ID" --status idle --timeout 180; then
  beam thread output "$DENY_THREAD_ID"
else
  beam thread show "$DENY_THREAD_ID"
fi
beam thread log "$DENY_THREAD_ID" --format json | jq '.[-12:]'
test ! -e "$DENY_FILE"
```
...
```bash
GRANT_THREAD_ID=$(beam thread spawn \
  --project "$BB_PROJECT_ID" \
  --provider claude-code \
  --model "$CLAUDE_MODEL" \
  --reasoning-level low \
...
for _ in {1..60}; do
  GRANT_INTERACTION_ID=$(beam thread interactions list "$GRANT_THREAD_ID" --json | jq -r '.[0].id // empty')
...
beam thread interactions show "$GRANT_INTERACTION_ID" "$GRANT_THREAD_ID"
beam thread interactions grant "$GRANT_INTERACTION_ID" "$GRANT_THREAD_ID" --scope turn
beam thread wait "$GRANT_THREAD_ID" --status idle --timeout 180
beam thread output "$GRANT_THREAD_ID"
...
- `accept-edits` turns allow workspace changes but surface pending interactions
  ...
  `beam thread interactions list/show`.
- `beam thread tell` reports the message as held while the thread is awaiting
