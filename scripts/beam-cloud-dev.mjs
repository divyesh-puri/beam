...
const DEV_BASE_DOMAIN = "beam.localhost";
...
function fail(message) {
  console.error(`Beam Cloud dev: ${message}`);
  process.exit(1);
}
...
function spawnService(args, env = {}) {
  ...
  child.once("error", (error) => {
    ...
    console.error(`Beam Cloud dev: ${error.message}`);
...
console.log(`
Local Beam Cloud is ready at ${BOLD}${CLOUD_URL}/dashboard${RESET}

Claim a handle, generate a pairing code, and run the command shown in the
dashboard against a Beam instance started with pnpm dev. Local handles use:
