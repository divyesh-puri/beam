...
const COMMAND_INDEX_PATH = fileURLToPath(
  new URL(
    "../../../server/src/services/skills/builtin-skills/beam-cli/references/command-index.md",
    import.meta.url,
  ),
);
...
const BB_CLI_SKILL_ROOT = fileURLToPath(
  new URL(
    "../../../server/src/services/skills/builtin-skills/beam-cli/",
    import.meta.url,
  ),
);
...
describe("beam-cli skill command index", () => {
  it("lists every core command path", async () => {
    ...
    const documented = new Set(
      [...index.matchAll(/^- `beam ([^`]+)`$/gm)].map((match) => match[1]),
    );
