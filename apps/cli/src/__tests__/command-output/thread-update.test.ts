...
describe("beam thread update command output", () => {
  setupCommandOutputTestEnvironment();

  const register: CommandRegistrar = (program) =>
    registerThreadCommands(program, () => "http://server");

  it("beam thread update sets the parent thread id", async () => {
  ...
  it("beam thread update changes visibility", async () => {
  ...
  it("beam thread update rejects invalid parent-thread values", async () => {
  ...
  it("beam thread update clears the parent thread id", async () => {
  ...
  it("beam thread update moves a thread into a section", async () => {
    const thread = fixtures.makeThread({
      id: "thread-section",
      projectId: "proj-1",
      providerId: "codex",
  ...
  it("beam thread update sets a sticky model and reasoning level override", async () => {
    const thread: domain.Thread = fixtures.makeThread({
      id: "thread-update-3",
      projectId: "proj-1",
  ...
  it("beam thread update sets the model override independently", async () => {
    const thread: domain.Thread = fixtures.makeThread({
      id: "thread-update-4",
      projectId: "proj-1",
      providerId: "claude-code",
  ...
  it("beam thread update rejects an invalid reasoning level before calling the API", async () => {
    const patch = vi.fn();
    stubServerApi({ "v1.threads.:id.$patch": patch });
