...
describe("beam guide command output", () => {
  setupCommandOutputTestEnvironment();

  it("beam guide unknown chapter lists available chapters", async () => {
    await expect(
  ...
  it("beam guide terminals documents explicit scopes and ID-only mutations", async () => {
    ...
    expect(output).toContain("beam terminal list --thread <thread-id>");
    expect(output).toContain("beam terminal rename <terminal-id> <title>");
  });
});
