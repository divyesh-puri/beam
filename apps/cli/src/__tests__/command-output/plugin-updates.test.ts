...
describe("beam plugin update commands", () => {
  setupCommandOutputTestEnvironment();
  ...
  it("outdated renders every outcome, reasons, and the dev-build annotation", async () => {
      ...
      jsonResponse({
        results: [
          ...
          },
          {
            ...
            devMode: true,
            installed: version("1.0.0"),
            blocked: { version: "2.0.0", reasons: ["requires Beam >= 9"] },
          },
          {
            id: "e",
            outcome: "unavailable",
    ...
    expect(output).toContain("2.0.0: requires Beam >= 9");
  ...
  it("list annotates only CLI commands shadowed by core", async () => {
    ...
    const output = collectLogPayloads(vi.mocked(console.log)).join("\n");
    expect(output).toContain(
      'command: beam plugin run shadower — Shadow threads (core command "beam thread" takes precedence)',
    );
    expect(output).toContain("command: beam notes — Take notes");
    expect(output).not.toContain('core command "beam notes"');
  });
  ...
  it("--all updates compatible results and skips pinned and incompatible", async () => {
        ...
        jsonResponse({
          results: [
            { id: "pin", outcome: "pinned", installed: version("1") },
            {
              ...
              outcome: "incompatible",
              installed: version("1"),
              blocked: { version: "2", reasons: ["requires newer Beam"] },
            },
            {
              id: "good",
              outcome: "update-available",
    ...
    expect(output).toContain(
      "bad: skipped — incompatible: requires newer Beam",
    );
