...
describe("beam thread output command output", () => {
  setupCommandOutputTestEnvironment();

  const register: CommandRegistrar = (program) =>
    registerThreadCommands(program, () => "http://server");

  it("beam thread output requires a thread id or --self", async () => {
  ...
  it("beam thread output --self resolves from BB_THREAD_ID", async () => {
    vi.stubEnv("BB_THREAD_ID", "thread-output-context");
    const getOutput = vi.fn(async () => ({ output: "FINAL" }));
    stubServerApi({ "v1.threads.:id.output.$get": getOutput });
  ...
  it("beam thread output --json prints the raw output payload", async () => {
    const getOutput = vi.fn(async () => ({ output: "FINAL" }));
    stubServerApi({ "v1.threads.:id.output.$get": getOutput });
