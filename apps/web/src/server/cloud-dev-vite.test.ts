...
const localEnv = {
  ...
  BB_CLOUD_DEV_APP_URL: "http://beam.localhost:42745",
  BB_CLOUD_DEV_SERVER_URL_TEMPLATE: "http://{label}.beam.localhost:42745",
...
describe("resolveCloudDevViteSettings", () => {
  ...
  it("configures local auth during the launcher-owned dev server", () => {
    expect(resolveCloudDevViteSettings("serve", localEnv)).toEqual({
      ...
      vars: {
        APP_URL: "http://beam.localhost:42745",
        BASE_DOMAIN: "beam.localhost",
        BETTER_AUTH_SECRET: "shared-local-secret",
        CONNECT_SERVER_URL_TEMPLATE: "http://{label}.beam.localhost:42745",
