...
export function formatSurface(surface: string): string {
  return surface === "desktop" ? "the Beam desktop app" : "a terminal";
}
...
export function renderExistingServerDialogHtml(
  ...
  const replaceButtonHtml = canReplace
    ? `<button type="button" data-choice="replace">Quit other Beam instance</button>`
    : "";
  ...
  const introText = canReplace
    ...
    : "This app can use the Beam instance that is already running. Beam cannot identify that process, so it cannot stop it for you.";
...
<head>
  ...
  <title>Beam is already running</title>
  <style>
    :root {
...
<body>
  <h1>Beam is already running on this Mac</h1>
  <p>${introText}</p>
  <div class="details">
      ${detailHtml}
  </div>
  ${replaceWarningHtml}
  <div class="actions">
    <button type="button" data-choice="quit">Quit this Beam instance</button>
    ${replaceButtonHtml}
