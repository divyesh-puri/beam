    ...
    async (context, payload) => {
      ...
      if (payload.protocolVersion !== HOST_DAEMON_PROTOCOL_VERSION) {
        ...
        deps.logger.error(
          ...
          "Rejecting daemon session: protocol version mismatch. An older auto-update-enabled daemon will install this server's Beam runtime package; a newer daemon requires the server to be updated.",
        );
        throw new ApiError(
          400,
