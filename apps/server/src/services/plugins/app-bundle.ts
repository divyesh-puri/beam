...
export function validatePluginArtifactMeta(args: {
  ...
  if (meta.sdkMajor !== PLUGIN_SDK_MAJOR) {
    return `${args.artifact} artifact for plugin "${args.pluginId}" was built for SDK major ${meta.sdkMajor}, running SDK major is ${PLUGIN_SDK_MAJOR}; rebuild the ${args.artifact} artifact with this Beam instance version`;
  }
