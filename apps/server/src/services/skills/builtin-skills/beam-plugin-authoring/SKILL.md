...
description: Write, build, test, and install Beam plugins. Use whenever a task creates or changes a Beam plugin, Beam CLI command, agent tool, background service, provider, setting, panel, mention provider, file renderer, message action, or other Plugin SDK surface.
---

# Author Beam plugins

A Beam plugin is a TypeScript package that can add server behavior, agent
...
routes to detailed references, but the installed Beam version decides the exact
API.
...
4. Build with beam plugin build.
5. Test the contract and the user workflow.
6. Install or reload only when the task requires a live check.

Use beam plugin new <name> for a new plugin. The scaffold includes frontend files.
...
- Run beam plugin types when SDK declaration versions can drift.
- Run beam plugin build before install, release, or marketplace submission.
