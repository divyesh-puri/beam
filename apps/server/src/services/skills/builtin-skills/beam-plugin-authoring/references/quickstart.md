...
```
beam plugin new hello            # scaffolds ./beam-plugin-hello: a todo list with a sidebar page, `beam hello` CLI, and a skill
cd beam-plugin-hello
beam plugin install .            # registers the directory in place (--yes to skip the prompt)
beam plugin dev                  # rebuild app/host bundles + reload on every save
```
...
- `bb.server` (required) — backend entry. Path installs load it as
  TypeScript directly (no build step); `beam plugin build` also emits a
  ...
  `beam plugin build` into minified `dist/app.js` + `app.css` + `app.meta.json`
  (`beam plugin dev` keeps them readable); path and git installs build it
  ...
  at runtime. Runtime imports that Beam does not shim belong in `dependencies`.
  ...
  and tooling only — including every package Beam shims at runtime (sonner,
  ...
  needs a `devDependencies` entry at the host's version (`beam plugin new`
  writes all of them; `beam plugin types` repins them). Never put one in
...
- Git installs use npm, then build declared app, server, and host source. They
  ...
  `PATH`.
- Building yourself (CI, or verifying a build without a running Beam): add
  `bb-app` to `devDependencies` and set `"build": "beam plugin build"`.
  `beam plugin build` needs no running server, but the manifest still needs
  `bb.server`. Depending on `bb-app@X` builds
  against exactly that release's shim configuration. Beam downloads its build
...
- `bb.branding` (required) — declare `bb.branding.icon` as either the plugin's
  canonical Beam icon name, such as `Zap`, or a plugin-relative compact SVG path
  ...
  Beam validates path-shaped SVGs (well-formed XML with an `<svg>` root, no
  ...
  colors are ignored.
  Beam reuses this icon on roomy surfaces when no logo override is declared.
  ...
  light logo fail the manifest. `beam plugin build` refuses an SVG logo that
  ...
  reject the invalid paths and SVGs described above. Every SVG Beam serves
  ...
  logo is required. Beam uses a declared logo where space permits, such as roomy
  ...
  `beam plugin reload`. Named inline icons use `currentColor`; compact SVG assets
...
- `bb.branding.experimental_icons` (optional) — the plugin's own icon
  ...
  `bb.providers.register` `icon`. Beam serves each file hashed from
...
- `engines.bb` — optional compatibility string checked during runtime and
  update selection against the Beam app version.
- `engines.bbPluginSdk` — optional SDK compatibility string. The scaffold uses
  the repository SDK version. Beam validates this range before use. Absent means
  ...
  Compatible updates (`beam plugin outdated` / `beam plugin update`) only select
  candidates that satisfy these ranges; newer incompatible releases are
  reported as blocked rather than applied. Dev builds (Beam `0.0.0`) skip
  enforcing `engines.bb` and annotate that on check results.
- **Manual updates:** `beam plugin outdated` checks tracking sources and
  `beam plugin update` applies compatible candidates (reinstall of an already
  ...
  users.
- `beam plugin build` stamps authoritative metadata into every declared
...
- Default to `beam-plugin-hello` for the package name. Scoped names such as
  `@acme/beam-plugin-hello` are also supported. The plugin id is the final
  package-name component minus the `beam-plugin-` prefix; legacy `bb-plugin-`
  names remain supported. Beam lowercases it, replaces non-alphanumeric runs
  with `-`, and trims separators. An empty result is invalid. Every bundled
  plugin id is reserved for its bundled source. The id namespaces routes,
  storage, settings, and CLI commands.
...
dependencies (and are bundled by `beam plugin build`).
...
Use `beam plugin reload <id>` only when the change or plugin requires it.
