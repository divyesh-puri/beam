...
devDependency (or run `beam plugin migrate`) before tests can import the harness.
...
await harness.lifecycle.dispose(); // abort services, hooks LIFO, close database; stale API calls throw
```
...
### Live loop against a running Beam

- `beam plugin dev` is the loop: save → rebuild declared `bb.app` and `bb.host`
  ...
  `dist/app.js` + `app.css`; `beam plugin build` and installs minify them.
- `beam plugin list` shows status, services, schedules (with last_error),
  handler stats, and the CLI command; `beam plugin logs <id> -f` follows
...
application/json" -d '{}' <server>/api/v1/plugins/<id>/rpc/<method>`,
  `beam <command> …` for the CLI, `beam plugin run <id> …` as the explicit form.
- Keep pure logic in plain functions/modules so it is unit-testable without
  a Beam server; the factory file should mostly wire registrations.

Beam Official plugins in `plugins/` (a Beam checkout):
...
- `github` — a gh-CLI-backed issue/PR browser in a single navPanel (with
  ...
  project setting, a `beam github` CLI command, and agent-spawn buttons.
...
- Settings saves do not reload healthy or degraded plugins; live `onChange`
  ...
  plugin is `needs-configuration`; `beam plugin reload <id>` remains available
...
- Schedules only fire while the plugin is loaded (rows are durable, the
  runner is not).
- CLI `run(argv)` argv excludes the command name; core Beam command names
  are reserved; workspace-sandboxed agent threads (Accept Edits / Approve
  for me) may fail to reach the Beam CLI when the provider sandbox blocks
...
- Handler stats shown by `beam plugin list` persist across reloads (reset on
  remove).
...
- The declarations you read are pinned to one SDK version, not a live view:
  ...
  ones from a vendored `types/*.d.ts` copy. Run `beam plugin types` before
  ...
  "Looking up the exact API". `beam plugin migrate` moves an older plugin off the
