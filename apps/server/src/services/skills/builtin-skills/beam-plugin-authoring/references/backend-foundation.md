...
`beam plugin logs <id> [-n N] [-f]`.

### bb.settings

`bb.settings.define(descriptors)` declares plain-data descriptors (rendered
in Extensions → Plugins and editable via `beam plugin config <id> set <key>
...
server's own loopback base URL (e.g. `http://127.0.0.1:48886`), which serves
...
migration from Beam-managed files. Do not write plugin state there. Use
...
Beam does this). Either way the daemon never resolves the SDK or private Beam
packages from the plugin at runtime.

Pure JavaScript dependencies are bundled. For external tools, use
`child_process` to probe or invoke tools on `PATH`. Beam V1 provides no
...
Beam Connect machine enrollment or its connected daemon reports that the local
