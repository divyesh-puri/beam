...
- `ThreadChat` — Beam's complete chat surface for an existing thread, rendered
...
- `experimental_ProviderModelPicker` — Beam's controlled provider, model, and
...
routing?, allowProviderChange?, align?, disabled?, className? }`, where `routing` is
  ...
  Beam's composers. Provider switches wait for the target provider's verified
...
- `experimental_PermissionModePicker` — Beam's controlled permission-mode
  picker. Props:
...
- `experimental_SourceCode` — Beam's source viewer. Props:
  ...
  null). Beam owns syntax highlighting, gutters, and the live code theme.
- `experimental_Diff` — Beam's diff viewer. Props:
...
className? }` —
  ...
  (default) or `"split"`. Beam normalizes the patch, so a GitHub REST patch or
  ...
  consistent with the patch, Beam enables expand-context controls between
...
- `Markdown` — Beam's chat-message markdown renderer (same typography,
...
- `UrlLink` — a real anchor whose ordinary HTTP(S) activation
  ...
  internal Beam routes in SPA history, preserves modifier clicks, copying,
...
- `experimental_FileLink` — a real anchor for an explicit live file target:
  ...
  current surface's shared Beam preview. Its lazy context menu offers the
...
- `experimental_NewThreadComposer` — Beam's complete compose surface for
...
className?, draftKey? }` — the `default*` props are SEEDS, not controlled
  ...
  project". That choice submits Beam's personal-project id in `projectId` (not
