...
experimental_openFilePreview(options), experimental_openFileExternally(options) }`.
  ...
  `openUrl` owns HTTP(S) only and returns false for schemes Beam
...
- `experimental_useCodeTheme()` → `{ mode, name, theme }` — the code theme Beam
  ...
  the same document Beam's own highlighter paints from. Reach for it ONLY when
  ...
  approximate the palette by reading Beam's CSS variables: `--canvas` / `--ink`
...
- Use a vendored Beam prompt icon-button recipe for native-matching action chrome
...
component kit is removed. The app module still exports focused Beam capability
...
- `beam plugin new` pre-vendors button, card, input, checkbox, dialog (plus
  ...
  Beam. Import via the `@/*` alias: `import { Button } from
"@/components/ui/button"` (tsconfig maps it; `beam plugin build` reads it).
- Add more with stock shadcn tooling: `npx shadcn add @bb/select
@bb/table` — the Beam registry carries the full stock set (~44 items:
  accordion, alert-dialog, calendar, chart, command, form, sheet, table,
  …), generated from the Beam app's own component source, so vendored code is
  version-matched to your Beam by construction. Edit the copies freely; they
...
- Never bundled (runtime-shimmed, import freely): react, the portaling
  ...
  (the scaffold declares all of them; `beam plugin types` repins them; `beam
...
- Everything else bundles from YOUR `node_modules` (hugeicons, lucide,
  ...
  after adding components (`beam plugin new` runs the first one; `shadcn add`
  installs each item's declared deps). Users of your prebuilt artifact need no
  npm. Managed source installs do.
- The old Beam extras (`EmptyState`, `PageBody`, `Spinner`) are
...
renderers never had the `experimental_Original` alias. Beam removes these aliases
in Beam 0.42.
