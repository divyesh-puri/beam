...
1. **`beam plugin types`**, run in the plugin directory (or given its path),
   syncs that plugin's SDK surface to the running Beam — no server needed. For a
   plugin that depends on the npm package it repins the exact
   `@get-bb/plugin-sdk` devDependency to this Beam's SDK version and brings the
   runtime-shimmed packages' type-only devDependencies (sonner, vaul, the
   portal radix families, ...) to the versions this Beam instance ships — adding any an
...
2. **Read the bundled declarations** — the authoritative surface, ~13,000
   ...
   - plugins scaffolded by a current Beam depend on the npm package, so after
     `npm install` read
   ...
   - plugins scaffolded before that still carry the root declaration in
     ...
     `bb.host` so the `/host` and `/testing/host` subpaths are present; `beam
...
3. **`git clone --depth 1 https://github.com/divyesh-puri/beam`** for host behavior or
...
```sh
beam plugin install ./beam-plugin-notes
beam plugin install npm:beam-plugin-notes@^1.0.0
beam plugin install https://github.com/acme/beam-plugin-notes
beam plugin install git:https://github.com/acme/beam-plugin-notes.git@main
beam plugin install git:https://github.com/acme/beam-plugin-notes.git@^1.2.0
```
...
Beam reads the repository's tags, installs the highest release the range allows,
and `beam plugin update` moves them to later releases in the same range.
...
Beam records the tag it installed together with the commit that tag pointed at,
...
```sh
beam plugin install git:https://github.com/acme/beam-plugins.git@main --plugin notes
beam plugin install git:https://github.com/acme/beam-plugins.git@main --subdirectory plugins/notes
beam plugin install path:. --plugin notes
```
...
```json
{
  "$schema": "https://raw.githubusercontent.com/divyesh-puri/beam/main/apps/web/public/schemas/marketplace.schema.json",
  "schemaVersion": 1,
  "name": "acme-plugins",
  ...
  "plugins": [
    {
      ...
      "source": {
        "git": {
          "url": "https://github.com/acme/beam-plugins.git",
...
last catalog Beam validated keeps serving. `name` is the marketplace's identity
...
manifest — Beam fetches and validates them server-side and serves them from its
own origin.
...
beam marketplace list
beam marketplace refresh acme-plugins
beam marketplace remove acme-plugins
```
...
installing from a marketplace that is not `bb-community`, Beam resolves and shows
...
Beam's own official plugins are separate: inclusion in the `bb-community`
marketplace is a Beam release decision, not part of the plugin authoring
