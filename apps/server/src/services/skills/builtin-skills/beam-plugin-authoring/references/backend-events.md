...
and counted in the plugin's handler stats (`beam plugin list`).
...
- `"local"` (default) — accepts no `Origin` header or a trusted Beam app origin.
  A non-GET mutation must use `application/json`. Use this mode for the Beam
  frontend.
- `"token"` — requires the per-plugin token (`beam plugin token <id>`;
...
- Semantics differ on throw: a service throwing `NeedsConfigurationError`
  ...
  in the schedule's `last_status`/`last_error` shown by `beam plugin list`.
...
if (!initial.apiKey)
  bb.status.needsConfiguration(
    "Set apiKey with `beam plugin config <id>`, then reload.",
  );
```
