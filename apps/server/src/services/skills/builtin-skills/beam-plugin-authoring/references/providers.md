...
bb.providers.register({
  ...
  // Sessionless maintenance support (each defaults to false) so Beam can skip
  ...
  capabilities: {
    ...
    supportsThreadArchive: false, // Beam mirrors archive/unarchive onto it
    supportsThreadRename: false, // Beam forwards renames to it
  ...
  // path (Beam then probes once per machine), "workspace" (the default) when
...
not in the manifest, so `beam plugin build` cannot check it — keep it free of
...
mask, so a monochrome mark follows the Beam theme (and the declared
`strings.iconTint`) with no frontend bundle — this is how every provider Beam
...
(The four first-party provider plugins ship no `app.tsx`: Beam vendors their
...
(`beam settings general providerOrder '["my-agent","codex"]'` and
`beam settings general defaultProviderId my-agent`).
...
host-dependent: Beam asks that provider's bridge for `provider/health` and lists
...
artifact cannot import Beam's private `@bb/*` workspace packages; an installed
...
mints every Beam turn and item id and constructs the canonical timeline
events.
...
your provider, and Beam writes `<dir>/<providerId>/<threadId>/<direction>.ndjson`
