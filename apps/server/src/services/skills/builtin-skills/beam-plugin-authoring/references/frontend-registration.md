...
React and the SDK are **never bundled** — `beam plugin build` shims them to
the host's shared runtime, so the bundle only works inside Beam.
...
The user can pin Beam's list or a specific provider under
...
If the chosen plugin is disabled, uninstalled, or its component throws, Beam
...
interface PluginThreadListProps {
  ...
  searchQuery: string;
  /** Beam's bound thread list. Render it to delegate conditionally without
...
// `indicator` (Beam's resolved status kind) + `indicatorLabel` (its a11y string).
// Draw your own glyph for `indicator`; the SDK ships no status component.
// Treat an unknown indicator value as "none" — Beam adds kinds over time.
...
actions.open(id, { split: true }); // Beam's split placement rules
...
actions.requestDelete(id); // opens Beam's delete confirmation
```

Destructive actions deliberately route through the host's own flow, so there
is no silent `delete`: deletion is recursive, and only Beam can show the
...
menu from `experimental_useSidebarThreadActions` — it exposes everything Beam's
own menu does, including `requestDelete`, which opens Beam's confirmation.

**Keyboard support is a DOM contract.** Beam's thread shortcuts find rows by
