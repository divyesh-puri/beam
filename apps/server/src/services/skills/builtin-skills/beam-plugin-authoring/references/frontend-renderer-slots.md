...
- `experimental_sourceCodeRenderer` / `experimental_diffRenderer` →
  replace Beam's source or diff renderer everywhere it draws supplied content:
  ...
  wins, and a missing, disabled, or crashing replacement falls back to Beam's
  renderer. Installing and enabling the plugin activates it, and the user can
  pin Beam's renderer or a specific provider under
...
Original }`. `experimental_fullFileContents` is either
  ...
  Every value is already resolved. Render `Original` (Beam's
  ...
  name for one release (it warns once; removed in Beam 0.42).
...
- `commandPaletteAction` → a row in Beam's quick palette (Mod+Shift+P), listed
  under "Plugins" beside Beam's own commands. Host-rendered chrome, no plugin
...
- `experimental_timelineRenderer` → the expanded body of the timeline rows a
  ...
  delegations, plan steps) always use Beam's renderers and are customized only
...
- `experimental_providerIcon` → the React component Beam draws as one agent
