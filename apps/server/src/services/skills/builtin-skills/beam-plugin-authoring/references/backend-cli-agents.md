...
### bb.cli — an agent-facing `beam` subcommand

One top-level command per plugin; a second `register` in one factory
execution is rejected.
Users and agents run `beam <name> …` like any core command; the Beam CLI
proxies it to the server, where `run` executes. Core collisions log an
activation warning and appear in `beam plugin list` as `beam plugin run <id>`.

```ts
bb.cli.register({
  name: "weather", // lowercase [a-z0-9-]+; core collisions use beam plugin run <id>
  ...
  commands: [
    // help/skill metadata only; parsing argv is yours
    {
      ...
      usage: "beam weather today <city>",
    },
  ],
  async run(argv, ctx) {
    // argv EXCLUDES the command name: `beam weather today sf` → argv = ["today", "sf"]
...
loopback, so `beam` CLI calls (including plugin commands) work sandboxed;
...
Beam remotely — share tunnel URLs"), use `contributeInstructions`:
...
bb.agents.registerTool({
  ...
  // Optional row presentation (grammar v3). Without it, Beam shows its normal
...
// selects this plugin's own ids when Beam resolves a thread/session config.
...
bb.agents.contributeInstructions(({ threadId, projectId }) => {
  ...
  return "The user is viewing Beam remotely — share tunnel URLs, not localhost.";
});
```
...
approval — falls back to Beam's standard `Running tool …` / `Ran tool …`
...
Ordering is standard Beam instructions, selected tools' static snippets,
...
takes effect only when the provider session is next started/resumed; Beam never
...
Beam's own AI services — the server-side helper completions behind thread
...
`BB_TRANSCRIPTION` set to `<id>/<model>` (`beam settings ai-services` lists
