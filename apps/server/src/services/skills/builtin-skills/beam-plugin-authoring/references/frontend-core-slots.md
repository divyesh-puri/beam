...
bundled JavaScript/TypeScript in the Beam app shell without a React slot. It is
...
code, so install only plugins you trust. Beam does not use `eval`, `Function`,
...
- `navPanel` → `{ subPath: string }` — owns the whole route at
  ...
  Beam automatically wraps every plugin page in the same host-owned App panel
  ...
  Terminal itself. Beam owns the desktop split, compact drawer, header/panel
  ...
  content tab closes an otherwise empty panel; if fixed tabs remain, Beam falls
  ...
  `experimental_target: { validate(value): value is Target }`; Beam checks JSON
...
- `sidebarFooterAction` → host-rendered icon button in the app sidebar footer
  ...
  `icon` is a Beam icon-name hint (unknown names fall back to a generic bolt).
- `fileOpener` → `{ path: string, source, Original }` — register as a viewer/editor
  ...
  by default. Users can pin Beam's preview or a specific opener per extension
...
projectId, experimental_hostId? }` (nullable fields). The optional host ID
  ...
  `Original` is Beam's preview bound to this file; render it to
