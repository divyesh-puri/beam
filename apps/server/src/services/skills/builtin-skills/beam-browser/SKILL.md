---
name: beam-browser
description: Use this when an agent must inspect or interact with a web application in Beam's visible Browser through reproducible CLI commands.
---
...
```sh
target=$(beam browser open "https://example.test" --json | jq -r .targetId)
trap 'beam browser close "$target" --json >/dev/null 2>&1 || true' EXIT
beam browser wait "$target" --text "Welcome" --json
beam browser snapshot "$target" --json
beam browser click "$target" --ref e0g1r3 --json
beam browser snapshot "$target" --json
