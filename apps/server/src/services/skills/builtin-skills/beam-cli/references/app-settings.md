...
- `beam settings general <key> <value>` accepts any key listed under
  `generalSettings` in `beam settings show`. Boolean preferences take `true`,
...
- `showKeyboardHints` defaults to true. Set it with
  `beam settings keyboard hints <true|false|on|off>` to control whether
...
- Reset removes the override and follows Beam's current default. Clear stores an
...
- The complete default table is in `docs/configuration.md` in the Beam source
  repository.

## Unhandled provider events

- `showUnhandledProviderEvents` defaults to false. Set it with
  `beam settings general showUnhandledProviderEvents <true|false|on|off>`.
- When enabled, packaged builds show raw provider events that Beam has persisted
...
- `steerActiveThreadOnEnter` defaults to false. Set it with
  `beam settings general steerActiveThreadOnEnter <true|false|on|off>`.
...
- `streamerMode` defaults to false. Set it with
  `beam settings general streamerMode <true|false|on|off>`.
- When enabled, every `customModels` entry from `~/.beam/config.json` is hidden
  in all model lists: the pickers, `beam provider models`, and
...
- The `providerSessionReaping` experiment defaults to false. When enabled, Beam
...
- The `mobileApp` experiment defaults to false while the Beam mobile app is in
  early access.
- Enable it with `beam settings experiment mobileApp true`. It shows the
  **Add mobile device** card under Settings → Remote access.

## Changelog preview

- The `changelogPreview` experiment defaults to false.
- Enable it with `beam settings experiment changelogPreview true` to show the
  latest release notes on Settings → Updates.

## Timeline windowing

- The `timelineWindowing` experiment defaults to false.
- Enable it with `beam settings experiment timelineWindowing true`.
