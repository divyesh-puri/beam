...
- A beam plugin is a TypeScript package running inside the Beam server, extending
  it with services, schedules, HTTP/RPC endpoints, settings — and `beam` CLI
  subcommands that agents run through bash like any other command.
- Use `beam plugin list` to inspect installed plugins and their current state.
- **Beam plugin catalog** (store under `/api/v1/plugin-catalog`):
  - The store lists the **Beam Community marketplace** catalog from a bundled
    snapshot without contacting a remote marketplace by default. Set
    `BB_MARKETPLACE_URL` to a public HTTPS manifest before startup to enable
    refreshes at startup and every two hours. Its entries install from their
    listed git or npm source through the normal install pipeline. A refresh
    only updates discovery metadata and icons; it never installs, updates, or
    runs plugin code, and a failed refresh keeps the last catalog Beam
    validated.
  - `beam plugin search <query> [--json]` — search the catalog by id,
    name, description, category, or tag; status shows installed / compatible /
    requires newer Beam. An **Installs** column appears once the curated
    marketplace's `stats.json` sidecar has been read (`installs` in `--json`,
    null when unknown): opt-in telemetry counts for published entries.
- **Third-party marketplaces** (routes under `/api/v1/marketplaces`):
  - `beam marketplace add <source>` — add a marketplace from an https manifest
    URL, `git:<url>[@<ref>]` (Beam reads `marketplace.json` from the checkout),
    or `path:<directory>`. The CLI resolves a relative path from its current
    directory before it sends the request. Beam validates the
  ...
  - `beam marketplace list [--json]` — name, source, entry count, last refresh.
  - `beam marketplace refresh [name] [--json]` — re-read one catalog or every
    one of them. Discovery metadata and icons only. A failed refresh keeps the
    last catalog Beam validated and exits non-zero.
  - `beam marketplace remove <name> [--json]` — forget a marketplace. Its
    ...
    resolution, so `beam plugin outdated`/`update` keep working from the
    recorded source.
  - Install a specific marketplace's entry with
    `beam plugin install <entry-id>@<marketplace>`. A bare entry id resolves
...
- Commands:
  - `beam plugin install <src>` — `<entry-id>@<marketplace>`, an HTTP(S) Git
    ...
    on its own. Beam records the selected tag and its commit and refuses to
    ...
    refused — use `beam plugin update`. Installing a local path for an id that
  ...
  - `beam plugin outdated` — check installed plugins for compatible updates
    (table; `--json` for raw results). Shows latest compatible candidate and
    any blocked incompatible newer release. Dev builds (Beam `0.0.0`) annotate
    that `engines.bb` is not enforced.
  - `beam plugin update <id>` / `beam plugin update --all` — apply compatible
    ...
    non-TTY refuses without it). Use `beam plugin outdated` to preview available
    updates. Changing a pinned git:/npm: source requires `beam plugin remove`
    ...
    place and `beam plugin reload <id>`, or `beam plugin install path:<new dir>`
    to move it; both keep its configuration.
  - `beam plugin list` — status, background services, schedules, handler timings,
    and each plugin's contributed `beam` command.
  - `beam plugin source <id> [--json]` — requested and resolved source, the
  ...
  - `beam plugin enable|disable <id>`, `beam plugin reload [id]` (exits 1 when a
    ...
    `beam plugin remove <id>` (deletes the plugin's settings, secrets, and
    schedules; managed git/npm files are deleted, and local path sources stay
    on disk).
  - `beam plugin config <id> [set <key> <value> | unset <key>]` — declared
    settings. Reload the plugin after configuring (`beam plugin reload <id>`).
  - `beam plugin token <id>` — print a short-lived bearer token for that plugin.
  - List, reload, enable, disable, config, and remove support `--json`.
  - `beam plugin logs <id> [-n N] [-f]` — the plugin's `bb.log` output.
  - `beam plugin run <id> [args...]` — explicit form; collisions log an activation
    warning and are annotated by `beam plugin list`.
  - `beam plugin new <name>` — scaffold a todo-list plugin (`server.ts`,
    `app.tsx` with a sidebar page, a `beam <name>` CLI command, a skill, and
    vendored UI components) and install its npm dependencies (scaffold sets
    `engines.bbPluginSdk` to `>=0.4.3`). The
    scaffold depends on `@get-bb/plugin-sdk`, pinned to this Beam's exact SDK
    ...
    rather than reporting success; `beam plugin build [path]` —
  ...
  - `beam plugin types [path]` — sync the plugin's `@get-bb/plugin-sdk` surface
    to the running Beam (default: cwd). For a plugin that depends on the npm
    package it rewrites the exact `devDependencies` pin to this Beam's SDK
    version and brings the type-only devDependencies of the packages Beam shims
    at runtime (sonner, vaul, the portal radix families, @pierre/diffs, clsx,
    tailwind-merge, class-variance-authority) to this Beam's versions — adding
    ...
    mismatch (for CI). `beam plugin build` and `beam plugin dev` refresh vendored
    declarations automatically and leave npm-package plugins alone. Needs no
    server.
  - `beam plugin migrate [path] [--yes]` — convert a plugin that still vendors
    `types/` to the `@get-bb/plugin-sdk` npm package (default: cwd): add the
    exact `devDependencies` pin, raise `engines.bbPluginSdk` when this Beam's SDK
  ...
  - `beam plugin dev [path]` — watch loop for an installed plugin (default:
    cwd): on every change it rebuilds the frontend bundle (when `bb.app` is
    declared; unminified, unlike `beam plugin build`) and reloads the plugin;
...
- **Writing a plugin?** Use the `beam-plugin-authoring` skill — the complete
  ...
  and gotchas. `beam guide plugins` has the short walkthrough.
