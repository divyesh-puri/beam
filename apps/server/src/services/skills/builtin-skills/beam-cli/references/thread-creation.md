...
- Use `beam thread spawn --project <project-id> --prompt "..."` to create another
  ...
  without a remembered model, Beam uses the explicitly requested provider or
...
- Use `beam thread fork <source-thread-id>` to clone a provider session. The
...
- Pass `--visibility hidden` for background/plugin workers that should remain
  ...
  attention. `beam thread list` excludes them by
  ...
  existing thread with `beam thread update <id> --visibility visible|hidden`.
- Stop a finished hidden worker with `beam thread stop <id>` to release its agent
...
- Add remote execution machines from Settings → Machines. Its one-line
  ...
  both the daemon protocol and agent-launched `beam` CLI to traverse the account
  gate; revoke a lost machine from the Beam Connect dashboard. The installer uses
  the server's exact `/install/bb-app.tgz` artifact and fails if that package is
  missing or unreachable; it never substitutes npm or a `PATH` executable. It
  installs under the enrollment's Beam data directory, without `sudo` or a
  global npm configuration, and enables daemon `--auto-update`.
  ...
  transient backoff, use `beam machine retry-update <id-or-name>`. Remove
  `--auto-update` from the service definition and reload it to opt out.
- Run `beam machine list` to see machine names, IDs, connection status, and last
  seen time (`--json` returns the raw host list). Use `--machine <id-or-name>`
  (alias `--host`) on `beam thread spawn` to run in a personal or unmanaged
...
- Each machine carries a permission limit (`maxPermissionMode`, default
  ...
  `beam machine list --json` or `beam machine show` and ask the user to change it
  in the app.
- `beam machine show`, `join-code`, `rename`, `retry-update`, and `remove` cover
  the Settings → Machines lifecycle. Use `beam machine provider-cli
status|install` to inspect or install provider CLIs on a selected machine.
- `beam updates` runs the default `beam updates status` action. It aggregates Beam and provider
  CLI update state across every machine — the CLI counterpart of Settings →
  Updates. `beam updates apply [--machine <id-or-name>]` runs every available
  provider CLI install/update sequentially; update Beam app itself with the
  printed release URL or the desktop relaunch.
- Use `beam project create --name <name> --root <path> --machine <id-or-name>`
...
- `beam project list` preserves the ordinary-project-only default. Pass
  `--include-personal` when the singleton personal project must be discoverable.
- Use `beam project source add <project-id> --machine <id-or-name> --path <path>`
...
- `beam project paths|files|content|commands` accept `--machine <id-or-name>`
  ...
  `beam project content --json` returns UTF-8 text or base64 binary content with
  an explicit `contentEncoding`.
- Use `beam project attachment upload <project-id> --client-file <path>` when the
  bytes live on the CLI machine, including when the CLI and Beam server are on
  ...
  (convert them to JPEG or PNG first). `beam project attachment download <project-id>
...
- `beam project history|reorder` exposes project prompt recall and sidebar order.
- Use `beam project show|update|delete` for one project. Use `beam project source
update|delete` for one source. Use `beam project branches` for branch data.
- Direct environment inspection accepts any environment ID: use `beam environment
status|branches|paths|diff|diff-files|diff-file|diff-patch <id>` and `beam
...
- `beam environment pull-request ready|draft|merge` manages pull-request state;
  `beam environment archive-threads` bulk-archives an environment's threads.
- Use `beam environment show|update|commit|squash-merge` for environment metadata
...
- If provider or model choice matters, inspect options with `beam provider list`
  and `beam provider models <provider-id>`. Both accept `--machine <id-or-name>`
...
- Cursor ACP threads discover project skills from `.cursor/skills`. This root
  can link to `.agents/skills`. `beam skill list` shows linked Cursor skills under
...
- Top-level `customModels` in the same `config.json` registers extra picker
  ...
  and Beam discovers it automatically. An OpenCode agent is a session mode, not
  a model, and cannot be selected through Beam. This list also has no set/unset
  CLI surface. Edit the JSON and restart Beam.
...
- Top-level `sharedSkillRoots` uses the same relative `user` and `project`
  paths. Beam lists these skills as read-only. Beam injects them into each provider,
  so one physical skill collection can support Beam and standalone provider CLIs.
