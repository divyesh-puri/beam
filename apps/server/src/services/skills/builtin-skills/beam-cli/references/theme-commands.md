...
- `beam theme` controls the **app-wide color palette** — a set of CSS-variable
...
- **Custom themes live on disk** under the app data dir, one folder per theme:
  `<beam-data-dir>/theme/<name>/theme.css` (Beam uses `~/.beam/theme/…`).
  The folder name _is_ the theme id. This mirrors how user skills live under
  `<beam-data-dir>/skills/<name>/`.
- All theme commands support `--json`.
- Commands:
  - `beam theme list` — built-in and custom themes and which palette is active.
  - `beam theme dir` — print the absolute custom-theme directory (where to create
    `<name>/theme.css`). Use this instead of guessing the path.
  - `beam theme set <id> [--favicon-color <color>]` — activate a built-in
  ...
  - `beam theme show [--css]` — print the active palette. For a custom theme,
    `--css` prints its CSS. For a built-in theme, it reports that Beam bundles
    the CSS.
  - `beam theme reset` — back to `default` while preserving the favicon color.
  - `beam theme favicon set <color>` — set the favicon color while preserving the
    active theme. Colors: `default`, `red`, `orange`, `yellow`, `green`, `teal`,
    `blue`, `purple`, `pink`.
  - `beam theme favicon reset` — reset the favicon color to `default` while
    preserving the active theme.

### Creating or editing a custom theme

This is the Beam habit: custom app-theme work belongs in
`<beam-data-dir>/theme/<name>/theme.css` — never a stray `.css` file elsewhere.

1. Find the directory: `beam theme dir` (e.g. `~/.beam/theme`).
...
3. Activate it: `beam theme set <name>`. Changes apply live to every open window.
