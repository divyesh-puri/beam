# beam theme authoring reference

How to write a good Beam app theme: the design model, the full design-token
...
The folder name is the theme id. Run `beam theme dir` to print the exact directory
...
and re-run `beam theme set <name>` (or just re-select it) to re-apply.
...
1. `beam theme dir` — print the custom-theme directory (e.g. `~/.beam/theme`).
2. Write your stylesheet to `<that-dir>/<name>/theme.css`. Start the name with
   ...
   hyphens.
3. `beam theme set <name>` — activate it. To edit later, change the file in place
   and re-run `beam theme set <name>`.
...
There is no separate code-theme setting. `beam theme show` prints the resolved
