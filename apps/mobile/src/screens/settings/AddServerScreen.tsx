...
export function AddServerScreen() {
    ...
    if (!probe.ok) {
      const where =
        probe.stage === "health"
          ...
          : "Reached the server, but it does not look like Beam";
  ...
  return (
    <>
      ...
      <Stack.Screen
        options={{
          title: firstRun ? "Connect to a Beam server" : "Add server",
        }}
      />
      <GroupedScreen testID="add-server-screen">
        <SettingsSection footnote="Pair through your Beam Connect service from anywhere: scan or type a pairing code from Beam Settings → Remote access.">
          <GroupedRow
            title="Connect with Beam Connect"
        ...
        >
          <View className="px-1">
            <Input
              ...
              placeholder="https://beam.example.ts.net"
              keyboardType="url"
              textContentType="URL"
              autoCapitalize="none"
              autoCorrect={false}
