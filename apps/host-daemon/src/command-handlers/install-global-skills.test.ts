  ...
  const entries = [
    {
      ...
      contentBase64: Buffer.from(
        `---\nname: ${name}\ndescription: Use ${name} outside Beam.\n---\n\n${body}\n`,
      ).toString("base64"),
    },
    {
...
describe("install global skills", () => {
  it("writes the skill into both global agent roots", async () => {
    ...
    const payload = createTreePayload("beam-cli", "first body");
    ...
    expect(result.installations.map((entry) => entry.path)).toEqual([
      path.join(homeDir, ".agents", "skills", "beam-cli"),
      path.join(homeDir, ".claude", "skills", "beam-cli"),
    ]);
  ...
  it("replaces a stale copy without leaving its removed files or staging dirs behind", async () => {
    ...
    await mkdir(path.join(claudeRoot, "beam-cli"), { recursive: true });
    await writeFile(path.join(claudeRoot, "beam-cli", "SKILL.md"), "stale\n");
    await writeFile(path.join(claudeRoot, "beam-cli", "dropped.md"), "gone\n");
    await mkdir(path.join(claudeRoot, "unrelated"), { recursive: true });
    await writeFile(path.join(claudeRoot, "unrelated", "SKILL.md"), "keep\n");
    const payload = createTreePayload("beam-cli", "fresh body");
    ...
    await expect(
      readFile(path.join(claudeRoot, "beam-cli", "SKILL.md"), "utf8"),
    ).resolves.toContain("fresh body");
    expect(await readdir(path.join(claudeRoot, "beam-cli"))).toEqual([
      "SKILL.md",
      "references",
    ]);
    expect(await readdir(claudeRoot)).toEqual(["beam-cli", "unrelated"]);
  });
  ...
  it("leaves the installed copy intact when the tree cannot be fetched", async () => {
    ...
    await mkdir(path.join(agentsRoot, "beam-cli"), { recursive: true });
    await writeFile(
      path.join(agentsRoot, "beam-cli", "SKILL.md"),
      "previous\n",
    );
    const payload = createTreePayload("beam-cli", "never arrives");
    ...
    await expect(
      readFile(path.join(agentsRoot, "beam-cli", "SKILL.md"), "utf8"),
  ...
  it("reports the installed hash as the tree hash, and detects drift", async () => {
    const dataDir = await makeTempDir();
    const homeDir = await makeTempDir();
    const payload = createTreePayload("beam-cli", "installed body");
    const command = {
      type: "host.install_global_skills" as const,
      skills: [
        { name: "beam-cli", treeHash: payload.treeHash, entryPath: "SKILL.md" },
      ],
    };
    const statusCommand = {
    ...
    await writeFile(
      path.join(homeDir, ".claude", "skills", "beam-cli", "SKILL.md"),
