...
the primary machine or another connected host, but the `beam docs` command
...
beam docs list --vault <vault-id> --json
beam docs read <path> --vault <vault-id>
```

Use the path and vault exactly as returned. Paths are relative to the vault;
do not guess an absolute host path or inspect the vault outside `beam docs`.
...
```sh
beam docs pull plans/release-plan.md --vault personal --into ./docs-work
# Edit ./docs-work/plans/release-plan.md with normal file tools.
beam docs status ./docs-work --diff
beam docs push ./docs-work
```

`beam docs status` exits 0 when no changes exist. It exits 4 when it finds
changes that the output describes. Exit 4 is a successful status result.
Review that output, then run `beam docs push` as a separate command. Do not
...
```sh
beam docs pull plans --folder --vault personal --into ./docs-work
...
before `push`. The manifest in `.beam-docs-state.json` records stable vault paths
