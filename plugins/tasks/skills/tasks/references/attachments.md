...
```sh
comment_id=$(
  beam tasks comment ABC-12 \
    --body "Screenshot of the failing step." \
    --json | jq -r '.comment.id'
)
beam tasks attachment add "$comment_id" --file ./screenshot.png
beam tasks attachment add "$comment_id" --file ./trace.log
```
...
Use repeatable `--attach <path>` with `beam tasks create` for initial files.

List IDs with `beam tasks attachment list <key>`. Remove a file with
`beam tasks attachment remove <attachment-id>`.
