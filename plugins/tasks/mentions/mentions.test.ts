...
describe("@task mention provider", () => {
  ...
  it("resolves complete task context including attachments and the CLI action contract", async () => {
    ...
    try {
      ...
      expect(context).toContain("Fetch with: beam tasks attachment get ");
      expect(context).toContain("Sawyer · User");
      expect(context).toContain("thr_worker · Mention worker · Working");
      expect(context).toContain(
        "You can act on this task with the beam tasks CLI.",
      );
      expect(context).toContain(
        "first run: beam tasks attach MEN-1 (attaches THIS thread so the task shows you as working)",
      );
      expect(context).toContain("beam tasks comment MEN-1 --body ...");
      expect(context).toContain("beam tasks update MEN-1 --status ...");
    } finally {
      await harness.dispose();
    }
  });
