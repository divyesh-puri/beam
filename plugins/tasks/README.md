...
Tasks is a Linear-style tracker inside Beam for planning work, delegating it to
...
Install Tasks from the official plugins that Beam includes:

```sh
beam plugin install tasks
```

The plugin adds the Tasks sidebar panel, the `beam tasks` command, and an agent
skill that teaches workers how to report progress back to tasks.

## Quick start

Install the plugin with `beam plugin install tasks`. Then use the `beam tasks` CLI
to create a tracker project. Link it to the Beam project where delegated agents
will run:

```sh
beam tasks project create \
  --name "Product" \
  --prefix PROD \
  --link-beam-project proj_your_beam_project
...
beam tasks list --project PROD
beam tasks show PROD-1
beam tasks preset list
beam tasks delegate PROD-1 --preset "GPT-5.6 · high"
```

When the CLI runs inside a linked Beam project, `create` and `list` infer the
...
choose or create presets under **Manage → Presets**, and type `@` in the Beam
...
`beam tasks comment PROD-1 --body "New context" --notify`.
...
Run `beam tasks --help` or `beam tasks <command> --help` for exact options. Add
...
| Command                                          | Purpose                                                                                                                                    |
| ------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------ |
| `beam tasks status`                              | Show the installed Tasks plugin name and version.                                                                                          |
| `beam tasks project create\|list\|show\|update`  | Manage tracker projects, folders, colors, prefixes, and Beam-project links.                                                                |
| `beam tasks folder create\|list\|update\|delete` | Organize tracker projects into nested folders. Deleting a folder moves its projects and subfolders to the top level; no tasks are deleted. |
| `beam tasks create`                              | Create a task with description, priority, labels, due date, optional parent, and file attachments (repeatable `--attach <path>`).          |
| `beam tasks list`                                | Page/filter tasks by project, status, priority, label, active agents, or search text; supports `--sort`, `--limit`, and `--cursor`.        |
| `beam tasks show <key-or-id>`                    | Show the complete task record, including comments, attachments, subtasks, and attached threads.                                            |
| `beam tasks update <key-or-id>`                  | Update status, priority, title, description, due date, or labels.                                                                          |
| `beam tasks comment <key-or-id>`                 | Add a Markdown comment from inline text or a file; optionally notify the latest responding task agent.                                     |
| `beam tasks attachment add\|get\|list\|remove`   | Add, fetch, list, or remove attachments. Referenced attachments require `remove --remove-references`.                                      |
| `beam tasks preset list\|create\|update\|delete` | Manage reusable agent execution presets.                                                                                                   |
| `beam tasks delegate <key>`                      | Start and attach a new agent thread using a preset.                                                                                        |
| `beam tasks attach <key-or-id>`                  | Attach the current Beam thread to a task when it was not delegated from Tasks.                                                             |
| `beam tasks detach <key-or-id>`                  | Detach the current Beam thread (or `--thread <id>`) from a task, for example a dead predecessor after a respawn.                           |
| `beam tasks threads <key>`                       | List the Beam threads attached to a task: live threads first, newest first.                                                                |
| `beam tasks label create\|list\|delete`          | Manage project-scoped labels.                                                                                                              |
| `beam tasks seed-demo --yes`                     | Create sample folders, projects, labels, tasks, and comments for evaluation.                                                               |
...
Linking a Tasks project to a Beam project enables delegation. Open a task, choose
...
Delegation creates a worker thread in the linked Beam project, attaches that
...
thread with `beam tasks attach KEY`. The inverse is `beam tasks detach KEY
...
page and `beam tasks threads` list live threads before completed or failed ones,
newest first.

## Task mentions

Type `@` in the Beam composer and select **Tasks** to search by task key or title.
...
action contract as context. Tasks linked to the current Beam project rank first.
