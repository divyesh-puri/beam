...
const runInputSchema = z
  .object({
    ...
    resumeRunId: z
      ...
      .nullable()
      .describe(
        "Run ID of a prior Beam workflow to resume from. Calls in the causally safe, longest unchanged prefix return cached results; the first edited, new, or concurrent call and everything after it run live. The prior run must be terminal and from the same project and environment.",
      )
      .default(null),
  })
  .strict();
...
export default async function plugin(bb: BbPluginApi) {
  ...
  bb.agents.registerTool({
    ...
    description:
      "Execute a workflow script that orchestrates multiple subagents deterministically. Workflows run in the background — this tool returns immediately with a run ID and a `previewDirective`. After a successful call, emit that directive exactly once on its own line (not in a code fence) so Beam renders live progress in chat. A completion notification is sent to the origin thread. Use `beam workflows status <run-id>` for a compact summary. For detailed history, redirect a bounded JSONL page from `beam workflows history <run-id> --cursor <call-index> --limit <1-100>` into `$BB_THREAD_STORAGE`, then inspect the file with normal filesystem tools.",
  ...
  bb.agents.configure((context) => {
    ...
    if (context.origin.pluginId === bb.pluginId) {
      return {
        ...
        skills: [],
        instructions:
          "You are starting as a Beam workflow worker. Follow the workflow prompt. Your final text IS the return value, not a human-facing message. If the prompt requests structured output, call bb_workflow_result exactly once at the end of your response.",
      };
    }
    return {
      ...
      skills: ["workflows"],
      instructions:
        "When bb_workflow_run succeeds, copy its previewDirective into your response exactly once as a standalone line. Do not wrap it in backticks or a code fence, and do not invent or edit the run ID. The directive renders live workflow progress in Beam chat. `beam workflows status <run-id>` returns a compact summary. For detailed history, redirect `beam workflows history <run-id> --cursor <call-index> --limit <1-100>` into a file under `$BB_THREAD_STORAGE`, then inspect that JSONL file with normal filesystem tools. Use each page record's `nextCursor` to continue.",
    };
  });
