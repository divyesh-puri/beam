...
function requireContext(ctx: PluginCliContext): {
  ...
  if (ctx.projectId === undefined || ctx.threadId === undefined) {
    throw new Error("This command must run inside a Beam project thread");
  }
...
function statusSummary(page: WorkflowRunInspectionPage) {
  ...
  return {
    ...
    history: {
      format: "jsonl",
      pageUsage: `beam workflows history ${run.id} --cursor 0 --limit ${DEFAULT_HISTORY_LIMIT}`,
      fileUsage: `mkdir -p "$BB_THREAD_STORAGE/workflows" && beam workflows history ${run.id} --cursor 0 --limit ${DEFAULT_HISTORY_LIMIT} > "$BB_THREAD_STORAGE/workflows/${run.id}.jsonl"`,
    },
  };
}
...
export function registerWorkflowCli(
  ...
  bb.cli.register({
    name: "workflows",
    summary: "Run and inspect durable Beam workflows",
    commands: [
      {
        name: "run",
        summary: "Start a workflow and return immediately",
        usage:
          "beam workflows run (--script '<javascript>'|--file <path>|--name <name>) [--args '<json>'] [--resume <run-id>]",
      },
      {
        name: "validate",
        summary: "Validate workflow source and literal model selections",
        usage:
          "beam workflows validate (--script '<javascript>'|--file <path>|--name <name>)",
      },
      {
        name: "status",
        summary: "Show a compact workflow run summary",
        usage: "beam workflows status <run-id>",
      },
      {
        name: "history",
      ...
      },
      {
        ...
        usage: "beam workflows list [--limit <1-50>]",
      },
      {
        name: "stop",
        summary: "Cancel a workflow run",
        usage: "beam workflows stop <run-id>",
      },
    ],
    async run(argv, ctx) {
      try {
        ...
        return failure(
          "Usage: beam workflows <run|validate|status|history|list|stop> [options]",
        );
      } catch (error) {
