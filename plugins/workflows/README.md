...
fresh Beam installations. It runs provider-independent JavaScript orchestration
inside QuickJS while delegating actual reasoning to ordinary Beam threads.
...
`beam workflows` CLI documented below. Provider and model discovery uses Beam's
built-in `beam provider` commands. Structured workers separately
...
line. Beam replaces the directive with a compact live run card in chat. The card
...
The panel shows every phase and worker, links attached workers to their Beam
...
and Beam theme tokens. Directive attributes and restored panel parameters are
...
not on the Beam server machine. Beam reads them through the environment `hostId`
...
Workflow workers use Beam's generic hidden-thread visibility. They remain
...
beam plugin build plugins/workflows
```

User-facing commands (run these from a Beam project thread) are:

```bash
beam workflows validate --script '<javascript>'
beam workflows validate --file .bb/workflows/review.js
beam workflows validate --name review
beam workflows run --script '<javascript>' --args '<json>'
beam workflows run --file .bb/workflows/review.js --resume <run-id>
beam workflows run --name review
...
with Beam's built-in commands above. Never infer ACP model IDs from a provider
...
Agent options accept native `label`, `phase`, and `schema` alongside Beam's
