...
export const CLAUDE_SKILL_PLUGIN_NAME = "beam-global-skills";
...
export function ensureClaudeSkillPlugin(args: {
  ...
  const manifest: ClaudePluginManifest = {
    ...
    name,
    version: "0.1.0",
    description: `Skills injected by Beam (${args.root.id}).`,
    author: { name: "Beam" },
    skills: "./skills",
  };
  writeFileSync(
...
export function createClaudeSkillPluginsRoot(baseDir = tmpdir()): string {
  ...
  return mkdtempSync(join(baseDir, "beam-claude-skill-plugins-"));
}
