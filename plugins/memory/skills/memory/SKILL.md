---
name: memory
description: Use durable Beam memory when prior project knowledge or cross-project user preferences can improve the current task, and save durable new learning through the `beam memory` CLI.
---
...
memories for the current Beam project. The index contains summaries only.

## Retrieve progressively

When a memory summary may be relevant, inspect it instead of guessing:

1. Search with `beam memory search "<query>" --scope all --json`.
2. Read the selected record with `beam memory get <id> --scope all --json`.
...
```bash
beam memory update <id> --expected-version <version> \
  --summary "<new summary>" \
  --details "<new details>" \
...
```bash
beam memory forget <id> --expected-version <version> \
...
The CLI uses Beam's loopback server, which Claude's macOS workspace sandbox
