...
Beam's official plugin for durable, progressively disclosed agent memory. It
provides:
...
duplicated or conflicting memories split between Codex, Claude Code, and Beam.
...
- CLI-only agent access through `beam memory` (no native agent tools);
...
Install Memory from the Beam Official catalog:

```bash
beam plugin install memory
beam plugin list
```
...
beam memory search "Turbo typecheck" --scope all --json
beam memory get <id> --scope all --json
```

Project writes take the invoking CLI's Beam project context. Global writes must
...
beam plugin dev ./plugins/memory
```
