...
Beam replaces that leaf with this plugin's React component, which:
...
3. Shows loading / error states and a header action that opens the source HTML
   file in Beam's sidebar workspace viewer.
4. Points a sandboxed iframe at Beam's path-shaped worktree preview route. This
   ...
   origin (no `allow-same-origin`) so scripts cannot access the Beam page, its
...
content, and files over 5 MiB are rejected. The iframe then uses Beam's existing
confined worktree preview route to serve the document and relative assets.

It ships with Beam and is reconciled through the builtin plugin lifecycle. Ship
