...
A plugin uses them to replace Beam's thread list with its own.
...
every sidebar looks, resizes, and collapses like Beam.
...
interface PluginSidebarThread {
   ...
   * through Beam's precedence. The plugin draws it however it likes.
   */
...
the host. A plugin gets Beam's precedence for free — attention before work,
...
context menu is the plugin's too — every item Beam's own menu offers (open, open
...
up a thread action Beam adds later, and it can drift from Beam's labels and
...
`layout` is data on purpose. Beam draws a small pane mini-map in its own rows;
...
host-internal. They are bound to Beam's own section model, which a replaced list
...
Beam already has a backend version of this. `bb.ui.registerThreadAction` puts a
...
must be a portalled popover, not an inline panel. `beam plugin build` shims the
...
Also note the vocabulary. Beam's in-turn subagents are activity on the parent
...
Beam's thread shortcuts already work by DOM query, not by React state.
`getSidebarThreadShortcutTargets` collects
`[data-sidebar-thread-shortcut-target]` elements and reads
`dataset.sidebarThreadId`.

So a plugin gets Beam's surface-specific numbered thread shortcuts,
...
function Row({
  ...
  // Per row, exactly as Beam's own ThreadRow does it.
...
thread shortcuts, and right-click still opens Beam's full menu.
...
- **No new persisted thread fields.** A plugin that needs per-thread state —
  ...
  concepts out of Beam's schema and out of the host-daemon protocol.
...
the user can pin Beam or one provider in client-local Settings. A crash or a
...
3. **Layout leak.** `layout.panes` exposes Beam's pane geometry. Confirm the
