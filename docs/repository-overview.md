...
| Package or app                                                      | Role                                                                                                       |
| ------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------- |
| [`packages/bb-app`](../packages/bb-app)                             | Internal compatibility package with the Beam launcher, bundled `beam` CLI entry, and public SDK export.    |
| [`apps/desktop`](../apps/desktop)                                   | macOS/Linux Electron shell that supervises the packaged runtime and loads the Beam web UI.                 |
| [`apps/app`](../apps/app)                                           | Web UI for inspecting projects, threads, environments, and running work.                                   |
| [`apps/mobile`](../apps/mobile)                                     | Native phone client (Expo; iOS first, Android next) for a Beam server over Direct URLs or Beam Connect.    |
...
| [`apps/cli`](../apps/cli)                                           | Scriptable `beam` CLI for users and agents.                                                                |
| [`apps/web`](../apps/web)                                           | Beam website plus Connect auth/dashboard (TanStack Start on Cloudflare Workers; deployment unprovisioned). |
