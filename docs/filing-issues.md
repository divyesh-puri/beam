...
- Beam version (`beam --version` or Settings → About) and how it was installed (desktop app, locally built package, `install.sh`, from source at commit).
...
- Anything unusual about the setup: remote machine, Beam Connect, worktree environment, custom ports or data dir, cgroup/memory limits.

### 3. Steps to reproduce

Numbered, copy-pasteable, minimal. Exact commands, exact UI clicks, exact prompt text. Include a tiny scratch repo or fixture if the bug needs one. Prefer `beam …` CLI steps over UI descriptions when both work; they are unambiguous.
...
Paste the actual output verbatim (terminal output, log lines, HTTP responses, `beam thread log` excerpts). For visual bugs attach a screenshot or short recording and say what to look at. Then state what you expected instead.

### 5. Evidence

- Logs from `~/.beam/logs/` (or the dev data dir printed by `pnpm dev`), trimmed to the relevant window.
- Thread ids (`thr_…`), project ids, environment ids when relevant. They are primary keys and let us query the database directly.
- If you cite code, use a permalink to a commit: `https://github.com/divyesh-puri/beam/blob/<sha>/<path>#L<n>-L<m>`. Branch links rot.
...
- Link the Beam thread or the investigation report that produced the finding.
