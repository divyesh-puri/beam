...
The CSS uses Beam's public `--canvas`, `--ink`, and `--accent` theme anchors.
Production plugins should vendor the Beam prompt icon-button recipe for action
...
```sh
beam plugin install ./examples/plugins/composer-customization
```
...
`beam plugin reload composer-customization`.
