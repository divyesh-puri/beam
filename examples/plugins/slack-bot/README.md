...
Mention the bot in Slack and it spawns a Beam thread in your configured project;
...
- **Settings** — `botToken` and `signingSecret` as `secret: true` (stored in
  ...
  `project` picker (stores the Beam project id).
...
- **`bb.sdk.threads.spawn`** — project-default environment; Beam fills in
  `origin: "plugin"` and `originPluginId: "slack-bot"` automatically, so
  spawned threads are attributed in the thread list.
- **`bb.storage.kv`** — maps Slack `thread_ts` → Beam thread id (and back), so
  follow-up mentions land in the same Beam thread.
...
1. Install the plugin:
   ...
   ```
   beam plugin install ./examples/plugins/slack-bot
   ```
...
2. Create a Slack app (https://api.slack.com/apps → "From scratch"):
   ...
   - **Event Subscriptions**: enable, subscribe to the `app_mention` bot
     ...
     Slack must be able to reach your Beam server (use a tunnel such as
...
3. Configure and reload:
   ...
   ```
   beam plugin config slack-bot set botToken xoxb-...
   beam plugin config slack-bot set signingSecret ...
   beam plugin config slack-bot set project proj_...
   beam plugin reload slack-bot
   beam plugin list        # slack-bot should show "running"
   ```

4. Mention the bot in a channel it has been invited to. `beam plugin logs
