...
This frontend-only enhancement adds a theme-aware focus ring to editable Beam
surfaces without rendering a React slot. Install it from this directory and
use `beam plugin dev` to exercise reloads.

The static rule lives in `app.css`, which is Beam's preferred path for plugin
