...
- **`bb.cli.register`** — a `beam docs` command. Agents run it through bash
  exactly like humans do (`beam docs search <query...>`); the handler runs
...
- **`bb.settings.define`** — a boolean (`caseSensitive`) rendered in Beam's
  settings UI and editable with `beam plugin config agent-enrichment`.
- **`bb.storage.kv`** — caches the last search (`beam docs last` prints it;
...
Dependencies: only `zod`, for the tool parameters. When Beam runs from a
source checkout the import resolves from Beam's own dependencies, so the
...
```
beam plugin install ./examples/plugins/agent-enrichment
beam plugin list
```
...
beam docs last
beam plugin config agent-enrichment set caseSensitive true
```
...
After editing sources, `beam plugin reload agent-enrichment`.
